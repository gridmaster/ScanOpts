...
BEGIN
	...
	SELECT [Id] 
	  ...
	  SELECT b.[StandardDeviation], t.[StandardDeviation], b.[StandardDeviation] / t.[StandardDeviation] AS 'Movement', b.*, t.*
	  FROM [ScanOpts].[dbo].[BollingerBands] b
	  JOIN #Table1 t on t.Symbol = b.Symbol 
	  WHERE convert(varchar(10), b.[Date], 126) = '2017-04-24' --@secondDate
	  --WHERE convert(varchar(10), b.[Date], 126) = @secondDate
	  AND b.[Close] > b.[UpperBand]
	  AND b.[StandardDeviation] / t.[StandardDeviation] > 1.3 --@movement
	  AND b.[Volume] > 10000
	  ORDER BY b.[Id] --desc
